'''
    Python file to implement the Treasure class
'''

class Treasure:
    '''
    Class to implement a treasure
    '''

    def __init__(self, id, size, arrival_time):
        '''
        Arguments:
            id : int : The id of the treasure (unique positive integer for each treasure)
            size : int : The size of the treasure (positive integer)
            arrival_time : int : The arrival time of the treasure (non-negative integer)
        Returns:
            None
        Description:
            Initializes the treasure
        '''

        # DO NOT EDIT THE __init__ method
        self.id = id
        self.size = size
        self.arrival_time = arrival_time
        self.completion_time = None

    # You can add more methods if required

    def get_priority(self, current_time):
        """Calculate the current priority of the treasure."""
        return current_time - self.arrival_time - self.remaining_size

    def add_last_processed_time(self):
        setattr(self, 'last_processed_time', self.arrival_time)

    def is_in_heap(self):
        setattr(self, 'is_in_heap', False)
    
    def initialize_remaining_size(self):
        setattr(self, 'remaining_size', self.size)
